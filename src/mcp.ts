...
export function createMcpServer(opts: McpOptions): McpServer {
	...
	server.tool(
		...
		"Search and filter logs. Supports level, service, project, branch, text search, and time ranges.",
		{
			...
			from: z.string().optional().describe("Start time (ISO 8601 or relative like '1h', '30m', '7d')"),
	...
	server.tool(
		...
		"Get logs surrounding a specific log ID. Useful for understanding what happened before and after a specific event.",
		{
			...
			before: z.number().optional().default(5).describe("Number of logs before the target (default 5)"),
			after: z.number().optional().default(5).describe("Number of logs after the target (default 5)"),
		},
		async (args) => {
			const result = await fetchJson(`${url}/query`, auth, {
				method: "POST",
				body: JSON.stringify({
					...
					params: [args.log_id - args.before, args.log_id + args.after, args.before + args.after + 1],
				}),
			});
			return textResult(result);
		},
